...
def validate_registration(username,email,password,role, users_list):
    ...
    if password_validator(password) != True:
        err.append("Password must contain A capital letter, a small letter, a number and a special character")
...
def validate_login(email, password, users_list):
    error = []
    registered_user = filter_item_detail(email,users_list)  # check whether the iser is registered 

    if is_empty([email,password])==True:
